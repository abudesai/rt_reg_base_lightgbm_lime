...
def get_trained_model(train_data, data_schema, hyper_params):
    ...
    train_X, train_y, inputs_pipeline, _, _ = preprocess_data(
        train_data, None, data_schema
    )
...
def preprocess_data(train_data, valid_data, data_schema):
    ...
    if valid_data is not None:
        valid_X = inputs_pipeline.transform(
            valid_data.loc[:, train_data.columns != pp_params["target_attr_name"]]
        )
